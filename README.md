# Hi, I am Myo Myint Kyaw 👋
### Senior Full Stack Web Developer

<p align="left">
  <a href="https://www.linkedin.com/in/myo-myint-kyaw-049698126/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:myomyintkyaw796@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://wa.me/959425030932" target="_blank">
    <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="WhatsApp" />
  </a>
</p>

---

### 🚀 About Me
A skilled **Full Stack Web Developer with over 7 years of experience** in designing, developing, and maintaining end-to-end web applications. I specialize in building Learning Management Systems (LMS), ERP solutions, news portals, multi-vendor e-commerce platforms, and online ticketing systems. Passionate about leveraging expertise in both front-end and back-end technologies to deliver scalable and high-performance solutions.

---

### 🛠️ Technical Skills

| Categories | Technologies & Tools |
| :--- | :--- |
| **Backend** | `PHP` `Laravel` `CodeIgniter` `Python` `Django` `Node.js` `REST API` |
| **Frontend & Mobile** | `React JS` `React Native` `JavaScript` `HTML5` `CSS3` |
| **Databases** | `MySQL` `MSSQL` |
| **DevOps & Servers** | `AWS EC2` `Apache` `Git & GitHub` `Windows Server` |

---

### 💼 Work Experience

#### 💻 Senior Web Developer 
**[Willtec Myanmar Co., Ltd.](https://www.willtec-myanmar.com/)** | *May, 2024 - Present*
* Led development of a large-scale Online Education Management System, focusing on high-availability and security.
* Managed deployment and infrastructure optimization on AWS.
* Developed robust Course Management, Student & Teacher Management modules, Scheduling System, and RBAC.
* Built custom Assessment Tools and complex certification/grading logic.

#### 💻 Mid-level Web Developer 
**[SpectrumSoft Technologies Company Limited](https://www.spectrumsoft-tech.com/)** | *Sep, 2022 - Apr, 2024*
* Designed and developed a high-availability E-commerce backend using Laravel, MySQL, and Windows Server, supporting concurrent B2C and B2B transactions.
* Built and maintained RESTful APIs for Orders, Products, Inventory, Customers, and Fulfillment with secure and performant data handling.
* Implemented payment gateway integrations, webhook processing, and database transactions.
* Optimized inventory management using MySQL locking, stock reservation, and reconciliation.

#### 💻 Junior Web Developer 
**[Myanmar ICT Solutions Limited](https://myanmarictsolutions.pro/)** | *Aug, 2020 - Aug, 2022*
* Designed and delivered full-stack web applications using Laravel, MySQL, REST APIs, and modern frontend frameworks for ERP, government, and E-commerce platforms.
* Integrated secure payment gateways, real-time notifications, and analytics to enhance system functionality.

#### 💻 Junior Web Developer 
**EI DORADO Company Limited** | *Dec, 2019 - Aug, 2020*
* Contributed to the development of a full-featured Online Ticketing & Booking System, implementing booking features for hotels, tours, flights, and transport.
* Integrated travel APIs and improved UI/UX to ensure real-time data, smooth navigation, and accurate bookings.

---

### 🎓 Education & Qualifications

* 🎓 **Bachelor of Engineering (IT)** — Technological University (Monywa), Myanmar (*2017 - 2019*)
* 🎓 **Bachelor of Technology (IT)** — Technological University (Monywa), Myanmar (*2012 - 2015*)
* 📜 **Professional Certificate in Web Technologies (PHP)** — India-Myanmar Center for Enhancement of IT Skills (IMCEITS)

---

### 📬 Connect with Me
* 📍 **Location:** Hlaing Township, Yangon, Myanmar
* 📧 **Email:** [myomyintkyaw796@gmail.com](mailto:myomyintkyaw796@gmail.com)
* 📞 **Phone / WhatsApp:** [+95 9425030932](https://wa.me/959425030932)
* 🌐 **LinkedIn:** [Myo Myint Kyaw](https://www.linkedin.com/in/myo-myint-kyaw-049698126/)

---

### 📊 GitHub Statistics

<div align="center">
  <!-- ၁။ Profile Big Card အပိုင်း -->
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=myomyintkyaw2280&theme=dark" alt="GitHub Profile Details" width="100%" max-width="850px" />
  <br /><br />

  <!-- ၂။ Profile Graph နှင့် Contribution Streak (ဘယ်ညာတွဲ Table Layout) -->
  <table width="100%">
    <thead>
      <tr>
        <th width="50%" align="center"><b>Profile</b></th>
        <th width="50%" align="center"><b>Contribution streak</b></th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td align="center">
          <img src="https://github-readme-stats.vercel.app/api?username=myomyintkyaw2280&show_icons=true&theme=dark&count_private=true&include_all_commits=true&hide_border=true" alt="Profile Graph" width="100%" />
        </td>
        <td align="center">
             <img src="https://github-readme-streak-stats.herokuapp.com/?user=myomyintkyaw2280&zone=Asia/Yangon&theme=dark&hide_border=true" alt="Contribution Streak" width="100%" />
        </td>
      </tr>
    </tbody>
  </table>
  <br />

  <!-- ၃။ Top Languages အပိုင်း (ဘယ်ညာတွဲ Table Layout) -->
  <table width="100%">
    <thead>
      <tr>
        <th width="50%" align="center"><b>Top languages (repos)</b></th>
        <th width="50%" align="center"><b>Top languages (commits)</b></th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td align="center">
          <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=myomyintkyaw2280&layout=compact&theme=dark&hide_border=true" alt="Top Languages by Repo" width="100%" />
        </td>
        <td align="center">
          <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=myomyintkyaw2280&layout=donut&theme=dark&hide_border=true" alt="Top Languages by Commit" width="100%" />
        </td>
      </tr>
    </tbody>
  </table>
  <br />

  <!-- ၄။ အောက်ဆုံးက ရက်အလိုက် လှိုင်းပုံစံပြမည့် Contribution Activity Graph -->
  <p align="center"><b>Contribution Activity</b></p>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=myomyintkyaw2280&theme=react-dark&bg_color=0d1117&hide_border=true" alt="Contribution Activity Graph" width="100%" />
</div>
